        ...
        <div class="row justify-content-center">
            <div class="col-md-11">
                ...
                <div class="card mb-4">
                    <div class="row g-0">
                        <div class="col-md-5">
                            @php $outrasImagens = $filme->imagens->reject(fn($img) => $img->pivot->poster); @endphp
                            @php $poster = $filme->imagens->firstWhere('pivot.poster', true); @endphp
                            @if ($poster)
                                <img src="{{ asset('storage/' . $poster->caminho) }}" class="img-fluid h-100 rounded-start"
                ...
                @if ($outrasImagens->isNotEmpty())
                    <h2 class="h4 mb-3">Mais imagens</h2>
                    <div class="d-flex flex-wrap gap-3 mb-4">
                        @foreach ($outrasImagens as $imagem)
                            <img src="{{ asset('storage/' . $imagem->caminho) }}" class="img-thumbnail"
                ...
                @foreach ($filme->ator as $ator)
                    <div class="card mb-3 shadow-sm">
                        <div class="d-flex align-items-center p-3 gap-3">
                            ...
                            @if ($foto)
                                <img src="{{ asset('storage/' . $foto->caminho) }}" class="rounded-circle"
